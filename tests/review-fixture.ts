...
export async function reviewFixture(): Promise<PhotoCapture> {
  ...
  const blob=await canvasBlob(raw), png=await canvasBlob(layer,'image/png');
  return {id:'testaufnahme',raw,layer,blob,url:URL.createObjectURL(blob),layerUrl:URL.createObjectURL(png),width:1200,height:1600,opacity:50,filename:'zeitblick-testaufnahme'};
}
