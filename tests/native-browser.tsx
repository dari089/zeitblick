...
const lenses=[{id:'main',label:'Hauptkamera · 24 mm',facing:'environment'},{id:'tele',label:'Telekamera · 70 mm',facing:'environment'},{id:'front',label:'Frontkamera',facing:'user'}];
let generation=0;
Object.assign(window,{ZeitblickAndroid:{
 startCamera(lens:string,quality:string,id:string){const current=++generation;report({start:lens||'main',quality});setTimeout(()=>{if(current!==generation)return;window.dispatchEvent(new CustomEvent('zeitblick-cameras',{detail:{cameras:lenses}}));window.dispatchEvent(new CustomEvent('zeitblick-camera-result',{detail:{requestId:id,id:lens||'main',width:1440,height:1920,facing:lens==='front'?'user':'environment',exposureMin:-6,exposureMax:6,exposureStep:1/3}}));},30);},
