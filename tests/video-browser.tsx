...
async function main(){
 ...
 Object.defineProperty(navigator.mediaDevices,'getUserMedia',{value:async()=>{const {c,draw}=synthetic('#209050'),stream=c.captureStream(30),timer=setInterval(()=>{if(stream.getTracks()[0].readyState==='ended')clearInterval(timer);else draw();},33);const track=stream.getVideoTracks()[0],originalSettings=track.getSettings.bind(track);let zoom=1;Object.defineProperty(track,'getCapabilities',{value:()=>({zoom:{min:1,max:5,step:.01}})});Object.defineProperty(track,'getSettings',{value:()=>({...originalSettings(),deviceId:'test-camera',zoom})});Object.defineProperty(track,'applyConstraints',{value:async(c:MediaTrackConstraints)=>{zoom=(c.advanced?.[0] as {zoom:number}).zoom;}});return stream;}});
 Object.defineProperty(navigator.mediaDevices,'enumerateDevices',{value:async()=>[{kind:'videoinput',deviceId:'test-camera',label:'Testkamera'}]});
