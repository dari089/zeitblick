...
export function AlignmentEditor({onBack}:{onBack:()=>void}){
 ...
 useEffect(()=>{
  ...
  if(!ready||!a||!b){setRendering(false);return;}
 ...
 },[a,b,ready,geometry,state]);
 function checkpoint(){setUndo(h=>[...h.slice(-29),structuredClone(stateRef.current)]);}
 function change(next:State){checkpoint();setState(next);setMessage('');}
 ...
 async function exportOutput(){
  if(!a||!b||!ready)return;job.current?.abort();const controller=new AbortController();job.current=controller;setBusy('Ausschnitt wird berechnet …');
  ...
  try{
   ...
   toast.success(animated?'Übergang gespeichert.':'Ausschnitt gespeichert. Originale bleiben unverändert.');
 ...
 return <div className="alignment-editor" role="dialog" aria-label="Bilder ausrichten">
  ...
  <footer className="align-footer">
   ...
   <small>{saved||'Privat · keine Cloud · Originale bleiben unverändert'}{ready?' · Freie Ränder im JPG werden dunkel gefüllt.':''}</small>
  </footer>
 </div>;
}
