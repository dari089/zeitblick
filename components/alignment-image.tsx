...
export function AlignmentImage({image,points,selected,onPick,onSelect,onMove,label,editable=false,region,onRegion}:{image:EditorImage;points:Point[];selected:number|null;onPick:(p:Point)=>void;onSelect:(index:number)=>void;onMove:(index:number,p:Point)=>void;label:string;editable?:boolean;region?:Crop;onRegion?:(crop:Crop)=>void}){
 ...
 useEffect(()=>{const node=ref.current!;const ob=new ResizeObserver(([e])=>{if(e.contentRect.width>0&&e.contentRect.height>0)setSize(old=>old.w===e.contentRect.width&&old.h===e.contentRect.height?old:{w:e.contentRect.width,h:e.contentRect.height});});ob.observe(node);return()=>ob.disconnect();},[]);
 ...
 function update(next:typeof pose){const z=Math.max(1,Math.min(8,next.z));poseRef.current={z,x:Math.max(-w*z/2,Math.min(w*z/2,next.x)),y:Math.max(-h*z/2,Math.min(h*z/2,next.y))};setPose(poseRef.current);}
 function end(e:React.PointerEvent,cancel=false){const g=gesture.current;if(!cancel&&g&&!g.moved&&pointers.current.size===1){if(g.marker>=0){if(!editable||g.marker!==selected)onSelect(g.marker);}else{const p=norm(local(e));if(p.x>=0&&p.x<=1&&p.y>=0&&p.y<=1)onPick(p);}}pointers.current.delete(e.pointerId);begin();if(gesture.current)gesture.current.moved=true;setFocus(null);}
 const viewPose=region?{x:0,y:0,z:1}:pose;
 return <div className="align-image">
  ...
  <div ref={ref} className="point-viewport" role="group" aria-label={label+' – Punkte setzen, mit zwei Fingern zoomen'} onPointerDown={e=>{
   if(region)return;e.currentTarget.setPointerCapture(e.pointerId);const p=local(e);pointers.current.set(e.pointerId,p);
   const index=points.findIndex(q=>Math.hypot(size.w/2+pose.x+(q.x-.5)*w*pose.z-p.x,size.h/2+pose.y+(q.y-.5)*h*pose.z-p.y)<22);
   begin(index);if(editable&&index===selected)setFocus(norm(p));
  }} onPointerMove={e=>{
   ...
   if(values.length===1&&g.marker>=0&&g.marker===selected&&editable){if(!g.moved)return;const q=norm(p);const bounded={x:Math.max(0,Math.min(1,q.x)),y:Math.max(0,Math.min(1,q.y))};onMove(g.marker,bounded);setFocus(bounded);return;}
   ...
   <img src={image.url} alt={label} draggable={false} style={{width:w,height:h,transform:`translate(-50%,-50%) translate(${viewPose.x}px,${viewPose.y}px) scale(${viewPose.z})`}} />
   {!region&&<svg width={size.w} height={size.h} aria-hidden="true">{points.map((p,i)=><g key={i} transform={`translate(${size.w/2+pose.x+(p.x-.5)*w*pose.z},${size.h/2+pose.y+(p.y-.5)*h*pose.z})`}><circle r={selected===i?15:12} fill={selected===i?'#d4ed88':'#152611'} stroke="#ecf6cf" strokeWidth="2"/><text textAnchor="middle" dy="4" fill={selected===i?'#152611':'white'} fontSize="12" fontWeight="bold">{i+1}</text></g>)}</svg>}
   {region&&onRegion&&<div style={{position:"absolute",width:w,height:h,left:(size.w-w)/2,top:(size.h-h)/2}}><AlignmentCrop crop={region} onChange={onRegion} drawNew label="Suchbereich"/></div>}
   {focus&&<canvas ref={loupe} width={120} height={120} className="point-loupe" />}
  </div>
  {!region&&<div className="point-zoom"><button onClick={()=>update({...pose,z:pose.z/1.5})} aria-label={label+' verkleinern'}>−</button><button onClick={()=>setPose({x:0,y:0,z:1})}>{pose.z.toFixed(1)}× · Ganzes Bild</button><button onClick={()=>update({...pose,z:pose.z*1.5})} aria-label={label+' vergrößern'}>+</button></div>}
 </div>;
}
