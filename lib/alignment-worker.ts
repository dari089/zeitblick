...
async function automatic(a:Pixels,b:Pixels){
 ...
 try{
  ...
  if(da.rows<12||db.rows<12)throw Error('Zu wenige klare Bilddetails. Bitte gemeinsame Punkte manuell setzen.');
  ...
  if(correspondences.length<12)throw Error('Keine sichere automatische Zuordnung. Setze vier oder mehr gemeinsame Gebäudepunkte.');
  ...
  if(inliers.length<12||inliers.length/correspondences.length<.4||coverage('a')<.07||coverage('b')<.07)throw Error('Die Treffer sind zu unsicher oder liegen zu eng zusammen. Bitte manuell ausrichten.');
  const m=Array.from(mat.data64F) as number[],aw=a.width-1,ah=a.height-1,bw=b.width-1,bh=b.height-1;
  const h=[m[0]*aw/bw,m[1]*ah/bw,m[2]/bw,m[3]*aw/bh,m[4]*ah/bh,m[5]/bh,m[6]*aw,m[7]*ah,m[8]].map(v=>v/m[8]);validateHomography(h);
...
scope.onmessage=async e=>{
 try{
  ...
  else if(job.kind==='auto'){scope.postMessage({ok:true,result:await automatic(job.a,job.b)});}
